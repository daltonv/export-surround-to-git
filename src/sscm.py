...
class SSCM:
    ...
    def __verify_surround_env__(self):
        ...
        with open(os.devnull, 'w') as fnull:
            ...
            p.communicate()
            if (p.returncode != 1):
                raise Exception("%s failed to run. Check your -i argument." %
                                cmd)
    ...
    def __get_sscm_output__(self, cmd, get_lines=True):
        ...
        if get_lines:
            output = [
                real_line for real_line in stdoutdata.splitlines() if real_line
                ]
        else:
            output = stdoutdata
    ...
    def lsremoved(self, item, branch, repo, get_lines=True):
        cmd = self.exe + ' property "%s" -b"%s" -p"%s" -d ' % (item, branch,
                                                               repo)
        if self.username and self.password:
            cmd += '-y"%s":"%s" ' % (self.username, self.password)
    ...
    def get(self, file, branch, repo, scratchDir, time_string=None, get_lines=True):
        ...
        cmd = (self.exe + ' get "%s" -b"%s" -p"%s" -d"%s" -f -i -e ' %
               (fixed_file, branch, repo, scratchDir))
        if self.username and self.password:
            cmd += '-y"%s":"%s" ' % (self.username, self.password)
        if time_string:
            cmd += '-s"%s" ' % time_string
